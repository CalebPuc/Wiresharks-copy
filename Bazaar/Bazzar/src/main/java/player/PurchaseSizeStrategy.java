...
/*
 * A strategy that maximizes the number of cards purchased in one turn.
 *
 * Searches all combinations of up to 4 exchanges followed by card
 * purchases, collects the best candidates by card count, then applies
 * tie-breaking rules from the spec to pick one.
 *
 ...
 */
public class PurchaseSizeStrategy implements Strategy {
    ...
    public TurnDecision takeTurn(TurnState turn, Equations equations) {
        ...
        for (List<ExchangeStep> exchanges : allExchangeSeqs) {
            ...
            for (TurnDecision candidate : candidates) {
                int ca = candidate.getPurchases().size();
                int cb = best == null ? -1 : best.getPurchases().size();
                if (best == null || ca > cb) {
                    best = candidate;
                } else if (ca == cb) {
